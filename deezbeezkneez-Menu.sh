...
while [ 1 ]; do
  ...
  "1)")
    ...
    if [ -e "/opt/plexguide/plexdrive.log" ]; then
      echo "Log exists"
    else
  ...
  "2)")
    ## create log file if does not exist
    if [ -e "/opt/plexguide/rclone-en.log" ]; then
      echo "Log exists"
    else
  ...
  "6)")
    ## create log file if does not exist
    if [ -e "/opt/plexguide/unionfs-encrypt.log" ]; then
      echo "Log exists"
    else
  ...
  "8)")
    ## create log file if does not exist
    if [ -e "/opt/plexguide/move-en.log" ]; then
      echo "Log exists"
    else
